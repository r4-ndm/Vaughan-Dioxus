fn main() {
    ...
    // Windows resource compilation needs a valid `.ico`. Linux/macOS do not; we still run
    // `tauri-build` there so capabilities / ACL / permissions are merged for `generate_context!`.
    #[cfg(windows)]
    {
        match std::fs::read("icons/icon.ico") {
            Ok(bytes) => {
                let looks_like_ico = bytes.len() >= 4
                    && bytes[0] == 0
                    && bytes[1] == 0
                    && bytes[2] == 1
                    && bytes[3] == 0;
