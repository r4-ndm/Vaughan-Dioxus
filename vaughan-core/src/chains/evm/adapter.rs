...
use crate::chains::{
    Balance, ChainAdapter, ChainInfo, ChainTransaction, ChainType, Fee, TokenInfo, TxHash,
};
...
impl ChainAdapter for EvmAdapter {
    ...
    async fn estimate_fee(&self, tx: &ChainTransaction) -> Result<Fee, WalletError> {
        let ChainTransaction::Evm(evm_tx) = tx;
    ...
    async fn send_transaction(&self, tx: ChainTransaction) -> Result<TxHash, WalletError> {
        let signer = self.signer.as_ref().ok_or_else(|| {
            WalletError::SigningFailed("No signer configured for EvmAdapter".into())
        })?;
        ...
        if let Some(gas_price) = evm_tx.gas_price.as_deref() {
            let gas_price = U256::from_str(gas_price).map_err(|_| {
                WalletError::InvalidAmount(format!("Invalid gas_price wei: {}", gas_price))
            })?;
        ...
        if let Some(max_fee) = evm_tx.max_fee_per_gas.as_deref() {
            let max_fee = U256::from_str(max_fee).map_err(|_| {
        ...
        if let Some(prio) = evm_tx.max_priority_fee_per_gas.as_deref() {
            let prio = U256::from_str(prio).map_err(|_| {
    ...
    async fn get_tx_status(&self, tx_hash: &str) -> Result<crate::chains::TxStatus, WalletError> {
        ...
        let bytes = hex::decode(h)
            .map_err(|_| WalletError::InvalidTransaction("Invalid tx hash hex".into()))?;
        if bytes.len() != 32 {
        ...
        match receipt {
            ...
            Some(r) => {
                ...
                let ok = r.status();
                Ok(if ok {
                    crate::chains::TxStatus::Confirmed
                } else {
                    crate::chains::TxStatus::Failed
                })
            }
        }
    }
    ...
    async fn get_transaction_history(
        ...
        address: &str,
        limit: u32,
    ) -> Result<Vec<crate::chains::TxRecord>, WalletError> {
        let net = get_network_by_chain_id(self.chain_id).ok_or_else(|| {
            WalletError::Other("No built-in explorer config for this chain id".into())
        })?;
